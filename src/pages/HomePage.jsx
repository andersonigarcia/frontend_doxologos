      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          {professionals.length === 0 ? (
            ...
            <>
              {canNavigateProfessionals && (
                <button
                  type="button"
                  onClick={() => navigateProfCarousel(-1)}
              ...
              <motion.div
                ...
                onKeyDown={handleProfessionalCarouselKeyDown}
              >
                  ...
                  >
                    <div className="relative h-64 overflow-hidden">
                      <img
                        ...
                        alt={prof.name}
                        src={prof.image_url || 'https://images.unsplash.com/photo-1603991414220-51b87b89a371?w=400&h=300&fit=crop&crop=face'}
                      />
                    ...
                    <div className="flex flex-col flex-1 p-6">
                      <div className="flex flex-col gap-2">
                        ...
                        {prof.crp && (
                          <span className="text-sm font-medium text-gray-500">CRP {prof.crp}</span>
                        )}
                      </div>
                      <p className="mt-3 text-sm text-gray-600 leading-relaxed line-clamp-4">
                        {prof.mini_curriculum || prof.description || 'Profissional dedicado ao cuidado integral do paciente com acolhimento e ética cristã.'}
                      </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
            {professionals.map((prof, index) => (
              <button
                key={prof.id}
                type="button"
                onClick={() => scrollToProfIndex(index)}
                className={`w-3 h-3 rounded-full transition-colors ${activeProfIndex === index ? 'bg-[#2d8659]' : 'bg-gray-300 hover:bg-gray-400'}`}
