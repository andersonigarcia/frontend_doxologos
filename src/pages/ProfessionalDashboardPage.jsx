...
import { ProfessionalSidebar } from '@/components/admin/ProfessionalSidebar';
...
const ProfessionalDashboardPage = () => {
    ...
    const handleEditEvent = (event) => {
        ...
        // TODO: Renomear showManualZoomFields → showMeetingLinkFields em refactor futuro
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                                    <div>
                                        <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                                            ...
                                            {userRole === 'admin' ? 'Gestão Operacional de Agendamentos' : 'Minha Agenda de Atendimentos'}
                                        ...
                                        <p className="text-xs text-slate-500 mt-0.5">
                                            {userRole === 'admin' 
                                                ...
                                                : 'Acompanhe seus próximos atendimentos, histórico de sessões e ganhos projetados.'}
                                        </p>
                                    </div>
                                                                    ...
                                                                    <table className="w-full text-xs text-left text-slate-700">
                                                                        <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-[10px] border-b border-slate-200">
                                                                            <tr>
                                                                                ...
                                                                                <th className="p-3">{userRole === 'admin' ? 'Financeiro (Preço / Repasse / Taxa)' : 'Valor do Repasse'}</th>
...

                        {
                            (userRole === 'admin' || userRole === 'professional') && (
                                <TabsContent value="professionals" className="mt-6">
                                    <Suspense fallback={<div className="p-8 flex justify-center items-center"><Loader2 className="w-8 h-8 animate-spin text-[#2d8659]" /></div>}>
                                        <div className={`grid grid-cols-1 gap-8 ${userRole === 'admin' ? 'lg:grid-cols-3' : 'max-w-4xl mx-auto'}`}>
                                            {userRole === 'admin' && (
                                            ...
                                            <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-6 ${userRole === 'professional' ? 'shadow-lg p-8' : ''}`}>
                                                ...
                                                <form onSubmit={handleProfessionalSubmit} className="space-y-8 text-sm">
                                                    
                                                    {/* SEÇÃO 1: Identificação e Acesso */}
                                                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                                                        <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                                                            <UserCircle className="w-5 h-5 mr-2 text-[#2d8659]" /> Identificação e Acesso
                                                        </h3>
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                            ...
                                                            {(userRole === 'admin' || !isEditingProfessional) && (
                                                                <div className="md:col-span-2">
                                                                    <label className="block text-xs font-medium mb-1 text-gray-600">Senha de Acesso</label>
                                                                    <input
                                                                        name="password"
                                                    ...
                                                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                                                        <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                                                            <MessageCircle className="w-5 h-5 mr-2 text-[#2d8659]" /> Contato e Mídia
                                                        </h3>
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                            ...
                                                            <div>
                                                                                ...
                                                                                try {
                                                                                    ...
                                                                                    img.onload = async () => {
                                                                                        ...
                                                                                        canvas.toBlob(async (blob) => {
                                                                                            if (!blob) { toast({ variant: 'destructive', title: 'Erro ao processar imagem' }); return; }
                                                                                            ...
                                                                                            const { data, error } = await supabase.storage.from('professional-photos').upload(filePath, blob, { cacheControl: '3600', upsert: false });
                                                                                            if (error) {
                                                                                                secureLog.error('Erro no upload:', error);
                                                                                                toast({ variant: 'destructive', title: 'Erro no upload', description: 'Não foi possível fazer upload da imagem: ' + error.message });
                                                                                                return;
                                                                                            }
                                                                                            const { data: urlData } = supabase.storage.from('professional-photos').getPublicUrl(filePath);
                                                                                            if (urlData?.publicUrl) {
                                                                                                setProfessionalFormData({ ...professionalFormData, image_url: urlData.publicUrl });
                                                                                                toast({ title: 'Upload concluído!', description: `Imagem salva (${Math.round(width)}x${Math.round(height)}px)` });
                                                                                            }
                                                                                        }, 'image/jpeg', 0.85);
                                                                                    };
                                                                                    img.onerror = () => toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível carregar o arquivo' });
                                                                                    img.src = URL.createObjectURL(file);
                                                                                } catch (error) {
                                                                                    secureLog.error('Erro:', error);
                                                                                    toast({ variant: 'destructive', title: 'Erro', description: error.message });
                                                                                }
                                                                            }
                                                                        }}
                                                                        className="w-full p-2 border border-gray-200 rounded-xl text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-[#2d8659] file:text-white hover:file:bg-[#236b47] file:cursor-pointer bg-white"
                                                                    />
                                                                </div>
                                                                ...
                                                                {professionalFormData.image_url && (
                                                                    <div className="flex items-center gap-3 mt-3 bg-white p-3 rounded-xl border border-gray-100">
                                                                        <img
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <Button type="button" variant="ghost" size="sm" onClick={() => setProfessionalFormData({ ...professionalFormData, image_url: '' })} className="mt-1 text-xs text-red-500 hover:text-red-700 hover:bg-red-50 h-7 px-2">
                                                                                Remover
                                                                            </Button>
                                                                        </div>
                                                                    </div>
                                                    ...
                                                    {/* SEÇÃO 3: Área de Atuação */}
                                                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                                                        <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                                                            <Briefcase className="w-5 h-5 mr-2 text-[#2d8659]" /> Área de Atuação e Especialidades
                                                        </h3>
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                            <div>
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    className="w-full input rounded-xl h-[13.5rem] resize-none"
                                                                ></textarea>
                                                            </div>
                                                            <div>
                                                                <label className="block text-xs font-medium mb-1 text-gray-600">Serviços que Atende</label>
                                                                <div className="border border-gray-200 rounded-xl p-4 h-[13.5rem] overflow-y-auto bg-white grid grid-cols-1 gap-1">
                                                                    {services.map(service => (
                                                                        <label key={service.id} className="flex items-center space-x-3 py-2 px-3 rounded-lg cursor-pointer hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-colors">
                                                                            <input
                                                                                type="checkbox"
                                                                                ...
                                                                                onChange={e => {
                                                                                    ...
                                                                                    if (e.target.checked) {
                                                                                        setProfessionalFormData({ ...professionalFormData, services_ids: [...currentServices, service.id] });
                                                                                    } else {
                                                                                        setProfessionalFormData({ ...professionalFormData, services_ids: currentServices.filter(id => id !== service.id) });
                                                                                    }
                                                                                }}
                                                                                className="checkbox w-4 h-4 rounded text-[#2d8659] focus:ring-[#2d8659]"
                                                                            />
                                                                            <span className="text-sm font-medium text-gray-700">{service.name} <span className="text-gray-400 font-normal ml-1">(R$ {service.price})</span></span>
                                                                        </label>
                                                                    ))}
                                                                </div>
                                                                <p className="text-xs text-gray-500 mt-2">Selecione os serviços vinculados a este profissional</p>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div className="flex gap-3 pt-2">
                                                        <Button type="submit" className="w-full md:w-auto bg-[#2d8659] hover:bg-[#236b47] rounded-xl px-8" disabled={isSavingProfessionalProfile}>
                                                            {isSavingProfessionalProfile && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
                                                            {userRole === 'admin'
                                                                ? (isEditingProfessional ? 'Salvar Alterações' : 'Criar Profissional')
                                                                : 'Salvar Alterações'
                                                            }
                                                        </Button>
                                                        {userRole === 'admin' && isEditingProfessional && (
                                                            <Button type="button" variant="outline" onClick={resetProfessionalForm} className="rounded-xl px-6">
                                                                Cancelar
                                                            </Button>
                                                        )}
                                                    </div>
                                                ...
                                                <section className="rounded-2xl border border-gray-100 p-5 space-y-4">
                                                    <div className="flex items-start justify-between gap-3">
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-gray-500">Informe o link do Google Meet para o evento. Os participantes receberão automaticamente.</p>
                                                        </div>
                                                        <label className="flex items-center gap-2 text-xs font-semibold text-gray-600">
                                                            <input
                                                                type="checkbox"
